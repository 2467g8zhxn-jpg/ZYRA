...
import { Camera, Mail, User, Shield, Zap, Trophy, Save, Loader2 } from "lucide-react";
    ...
    <DashboardLayout>
      <div className="max-w-3xl mx-auto space-y-8 font-body">
        <div className="flex flex-col gap-2">
          <h2 className="text-3xl font-bold tracking-tight text-foreground flex items-center gap-2"><User className="h-8 w-8 text-accent" /> {t.nav.profile}</h2>
          <p className="text-muted-foreground">{t.settings.acc_desc}</p>
        </div>

        <div className="grid gap-8 md:grid-cols-3">
          <Card className="md:col-span-1 border-border overflow-hidden flex flex-col items-center p-8">
            <div className="relative group cursor-pointer" onClick={handlePhotoClick}>
              <Avatar className="h-32 w-32 border-4 border-accent/20">
                ...
                <AvatarFallback className="bg-accent text-accent-foreground text-4xl font-black">{profile?.nombre?.charAt(0) || "Z"}</AvatarFallback>
              </Avatar>
            ...
            <div className="mt-6 text-center">
              <h3 className="text-xl font-bold text-foreground">{profile?.nombre}</h3>
            ...
            {!isAdmin && (
              <div className="mt-8 w-full space-y-4">
                <div className="flex items-center justify-between p-3 bg-muted/30 rounded-xl border border-border"><div className="flex items-center gap-2"><Zap className="h-4 w-4 text-accent" /><span className="text-xs font-bold text-foreground">{t.dashboard.level}</span></div><span className="text-sm font-black text-accent">{profile?.nivel || 1}</span></div>
                <div className="flex items-center justify-between p-3 bg-muted/30 rounded-xl border border-border"><div className="flex items-center gap-2"><Trophy className="h-4 w-4 text-yellow-500" /><span className="text-xs font-bold text-foreground">{t.dashboard.points}</span></div><span className="text-sm font-black text-yellow-500">{profile?.puntos || 0}</span></div>
              </div>
            )}
          </Card>
          ...
          <Card className="md:col-span-2 border-border">
            <CardHeader>
              <CardTitle className="text-foreground text-lg">{t.employees.payroll}</CardTitle>
            </CardHeader>
